...
int main(int, char *[])  {
    ...
    DataFrameWrapper<int> dfw; // modify the type template according to your index column (if not an already formatted dataset, it will be assigned an int type to the first column, usually the Row index)
    dfw.loadAndReadFile();
    ...
    /* /1* std::cout << frequencyCounts << std::endl; *1/ // TODO solve bug that srops anything else from printing */
